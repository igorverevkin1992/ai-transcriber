...
# --- Startup recovery ---
# Автоматически возобновлять незавершённые задачи (queued/downloading/...) при
# старте сервера. Полезно для серверного/Docker-деплоя (восстановление после
# краша), но при ручном закрытии локального бэкенда приводит к нежелательному
# авто-перезапуску. false → «висящие» задачи помечаются ERROR и не запускаются.
RECOVER_INFLIGHT_ON_STARTUP = os.getenv("RECOVER_INFLIGHT_ON_STARTUP", "true").lower() in ("1", "true", "yes")
