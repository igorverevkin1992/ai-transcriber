...
- **Shoot passport** (`backend/passport.py`): an optional `.docx` "паспорт съёмки" uploaded alongside the video (`passport` field in `/batch/upload`) carrying the shoot description, hero (guest) names, and their count. It is **authoritative over the filename**: hero names → guest legend, hero count → pyannote hint `min=heroes+1 .. max=heroes+2` (guests + off-camera interviewer + a crew slack slot that gets folded into tech moments), description → context for `SPEAKER_BOUNDARY_CORRECTION`. Parsed deterministically by form labels (`parse_passport`: «Герои»/«Количество»/«Описание» in paragraphs or a 2-col table), with a Gemini fallback (`gemini_extract_passport`) when labels aren't recognized. No glossary/terms in the passport (those stay on `GLOSSARY_REPLACEMENTS`/env). The interviewer isn't a hero, so they stay `АЗК`. Wired through `process_uploaded_file_task` → `_transcribe_with_whisperx` (hint) + `_process_recognition_result` (meta merge + boundary correction). The deterministic parser's field labels need calibrating against the real passport template.
