...
- `SPEAKER_BOUNDARY_CORRECTION` — fix whole-turn diarization mislabels via Gemini (default: false, opt-in; needs `GEMINI_API_KEY`). pyannote occasionally assigns a full short turn to the wrong speaker (a guest's answer glued into the interviewer's paragraph or vice versa) — model-level error that count hints and word-level resegmentation don't catch. When true, a pass runs after `build_turns` (before legend/name finalization): it feeds the labelled dialogue + roles (interviewer `АЗК`, guests = hero names, topic from the shoot passport) to Gemini and reassigns ONLY high-confidence mislabels, then re-merges adjacent same-speaker turns. Degrades to no-op without Gemini. Logic: `correct_speaker_boundaries` (`backend/postprocess.py`), wired in `_process_recognition_result`.
